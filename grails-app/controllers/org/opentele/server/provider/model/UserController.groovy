...
class UserController {
    ...
    @Secured(PermissionName.USER_READ_ALL)
    def index() {
    ...
    @Secured(PermissionName.USER_CREATE)
    def create() {
    ...
    @Secured(PermissionName.USER_READ)
    def show() {
    ...
    @Secured(PermissionName.USER_WRITE)
    def edit() {
        def userInstance = User.get(params.id)
        if (!userInstance) {
            flash.message = message(code: 'default.not.found.message', args: [message(code: 'user.label', default: 'User')])
    ...
    @Secured(PermissionName.USER_DELETE)
    def delete() {
        def userInstance = User.get(params.id)
        if (!userInstance) {
			flash.message = message(code: 'default.not.found.message', args: [message(code: 'user.label', default: 'User')])
