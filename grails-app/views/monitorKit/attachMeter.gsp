...
<title><g:message code="monitorkit.add.meter.label" /></title>
</head>

<body>
	<div id="edit-monitorKit" class="content scaffold-edit" role="main">
		<h1><g:message code="monitorkit.add.meter.label" /></h1>
		...
		<g:if test="${meters.size > 0 }">
			<g:form method="post">
				...
				<div class="fieldcontain">
					<label for="addMeter">
                        <g:message code="monitorkit.add.meter.label" default="Department" />
					</label>
					<g:select name="meter.id" from="${meters}" value="${name}" optionKey="id" />
				</div>
				<fieldset class="buttons">
					<g:actionSubmit class="save" action="attachMeter" value="${message(code: 'default.button.save.label')}" />
				</fieldset>
			</g:form>
		</g:if>
		<g:else>
			<div class="fieldcontain">
				<g:message code="monitorkit.no.available.meters" />
			</div>
		</g:else>
	</div>
</body>
